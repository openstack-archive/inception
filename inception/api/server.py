...
class AsyncOrchestrator(threading.Thread):
    ...
    def _destroy(self):
        ...
        # for this thread.
        self.ic = self.session.query(
            InceptionCloud).filter(InceptionCloud.id == self.id).first()
...
class InceptionCloud(_BASE):
    ...
    def save(self, orch):
        ...
        self.gw_floating_ip = orch._gateway_floating_ip
    ...
    def to_dict(self):
        ...
        return {
            ...
            'image': self.image,
            'gw_id': hex_or_none(self.gw_id),
            'gw_floating_ip': self.gw_floating_ip,
            'chef_id': hex_or_none(self.chef_id),
            'chef_ip': self.chef_ip,
            'controller_id': hex_or_none(self.controller_id),
...
class OrchestratorAdapter(object):
    ...
    def create(self, environ, start_response, route_vars):
        ...
        try:
            ...
            ao = AsyncOrchestrator(opt_dict, 'create')
            ao.start()
    ...
    def delete(self, environ, start_response, route_vars):
        ...
        try:
            ...
            ao = AsyncOrchestrator(opt_dict, 'destroy', inception_cloud.id)
            ao.start()
    ...
    def show(self, environ, start_response, route_vars):
        ...
        response_headers = [('Content-type', 'text/json')]
        start_response(status, response_headers)
        return [anyjson.serialize(dict(cloud=opt_dict))]
